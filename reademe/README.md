...
## 命令列表：
    ...
    #开启本地开发服务器，监控项目文件的变化，实时构建并自动刷新浏览器
    ffe dev

    #开启本地开发服务器，代理后端接口，用于联调
    ffe dev -p

    #构建项目，构建好的文件会输出到 "dist" 目录，
    ffe build
...
项目基于 spa 方式实现前后端分离， node.js 将所有 url 都返回到同一个 html 页面（由前端提供），此 html 页面也是前端的入口页面。然后路由由前端控制（基于vue-router），根据不同的 url 加载相应数据和组件进行渲染。
...
使用 webpack 的 DefinePlugin 插件，在 config 目录下的 *.env.js 注入后端接口的环境， 联调的时候，只需通过 -p 参数运行 ffe dev 脚本即可，所有 mock 目录下定义的接口将会转发到 process.env.apis 中指定的环境：

    # dev.env.js 中设置后端接口环境
...
这样，所有 /bestbuy-app-server-cloud-server 这个路径的请求 ，将会转发到 https://app.renyimai.com.cn 这个环境
